...
const Leaderboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (data.success && Array.isArray(data.topUsers)) {
          // Map 'count' from backend to 'points' for the frontend UI
          const mappedData = data.topUsers.map(user => ({
            ...user,
            points: user.count || 0 
          }));
          setStats(mappedData);
  ...
  const getBadge = (points, index) => {
    // Points check zaroori hai taake 0 points wale ko badge na miley
    ...
    if (points > 10) return { name: 'Elite Member', color: 'text-[#00f3ff]', icon: <Star className="w-4 h-4" /> };
    return null;
  };
    ...
    <div className="min-h-screen bg-[#0f0c29] p-8 flex flex-col items-center">
      <div className="text-center mb-16 mt-10">
        <h1 className="text-6xl font-black text-white italic uppercase tracking-tighter">
          Community <span className="text-[#00f3ff] underline decoration-[#ff007a]">Heroes</span>
        </h1>
      ...
      <div className="max-w-4xl w-full bg-[#1a1a4b]/50 border-2 border-white/5 rounded-[40px] p-10 shadow-2xl backdrop-blur-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#ff007a]/5 blur-[100px] -z-10"></div>
        
        <div className="grid gap-6 relative z-10">
          {loading ? (
            <div className="text-center py-20 text-white font-black animate-pulse uppercase italic tracking-widest">
            ...
            stats.map((stat, index) => {
              ...
              return (
                <div key={index} className="flex justify-between items-center bg-white/5 p-8 rounded-[30px] border-2 border-white/5 hover:border-[#ff007a] transition-all duration-500 group">
                  <div className="flex items-center gap-8">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-2xl font-black text-white uppercase italic tracking-tighter group-hover:text-[#00f3ff] transition-colors">
                          {stat.name}
                        </span>
                        {badge && (
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-gray-500 uppercase font-black tracking-[0.2em]">
                          {stat.email ? stat.email.split('@')[0] : 'Network Member'}
                        </span>
                        <div className="w-1 h-1 rounded-full bg-white/20"></div>
                        <span className="text-[10px] text-[#ff007a] uppercase font-black tracking-[0.2em]">Verified Hero</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    <div className="flex items-baseline justify-end gap-1">
                      <span className="text-[#00f3ff] font-black text-5xl italic leading-none">{stat.points}</span>
